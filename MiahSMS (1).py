...
BOT_TOKEN = "8804910755:AAHXPNzT-RqvJTY2sQLtz6VQbMoq9IMYVj4"
API_KEY = "MIAHSMS_B3B951A3017115BDA304F76B"
BASE_URL = "https://miahsms.com/bot"           # আপনার প্যানেল ডোমেন (trailing slash ছাড়া)

# ---------- একটাই ডাটাবেজ ফাইল — নিচের প্রতিটা নাম এখন আলাদা .json ফাইল নয়,
# বরং database.db-এর ভেতরে একটা "সেকশন" (আগের মতোই ফাংশন কল করলেই হবে, ভেতরে কিছু বদলাতে হবে না) ----------
DATABASE_FILE = "database.db"
USER_DATA_FILE = "users"
PAID_SMS_FILE = "paid_sms"
STATS_FILE = "user_stats"
REFERRAL_DATA_FILE = "referral_data"
BANNED_USERS_FILE = "banned_users"
WITHDRAW_DATA_FILE = "withdraw_requests"
...
OTP_GROUP_ID = -1003937969013

# ==================== সব লিংক/ইউজারনেম এক জায়গায় (আগে কোডের মাঝখানে ছড়ানো ছিল) ====================
SUPPORT_LINK = "https://t.me/Airdopprovider"          # আপনার সাপোর্ট লিংক দিন
DEVELOPER_LINK = "https://t.me/Nakibul1"     # আপনার ডেভেলপার লিংক দিন
PANEL_LINK = "https://t.me/MiahHostOtp_bot"             # "‼️ বটের ইউজার নাম PANEL" বাটনে ব্যবহৃত
CHANNEL_LINK = "https://t.me/Airdopprovider"            # "📢 CHANNEL" বাটনে ব্যবহৃত
OTP_GROUP_LINK = "https://t.me/Grab_Instant_Otp"         # "📢 OTP GROUP" বাটনে ব্যবহৃত (৪ জায়গায় ছিল, একটাতে ভুল আলাদা লিংক ছিল — ঠিক করে একই লিংকে আনা হয়েছে)

# ==================== WELCOME MESSAGE CONFIGURATION ====================
WELCOME_MESSAGE = """✨ Welcome MiahSMS by MiahHost ✨ 
━━━━━━━━━━━━━━━━━━━━━━
...
# ==================== একটাই ইউনিফাইড ডাটাবেজ (database.db — SQLite) ====================
# users, paid_sms, user_stats, referral_data, banned_users, withdraw_requests,
# activity_logs, datarange — এই ৮টা সেকশনই একটামাত্র SQLite ফাইলে (DATABASE_FILE) থাকে।
# আলাদা কোনো .json ফাইল আর তৈরি হয় না।

_DB_SECTION_DEFAULTS = {
    BANNED_USERS_FILE: [],
    ACTIVITY_LOGS_FILE: [],
}
...
def load_data(section=USER_DATA_FILE):
    default = _DB_SECTION_DEFAULTS.get(section, {})
    conn = _get_db_connection()
    try:
        row = conn.execute("SELECT data FROM sections WHERE section = ?", (section,)).fetchone()
        if row is None:
            return default
        try:
...
def save_data(data, section=USER_DATA_FILE):
    conn = _get_db_connection()
    try:
        conn.execute(
            "INSERT INTO sections (section, data) VALUES (?, ?) "
...
# ==================== USER DATABASE ফাংশন (এখন database.db-এর "users" সেকশন) ====================
...
# ==================== STATS ফাংশন (এখন database.db-এর "user_stats" সেকশন) ====================
...
# ==================== অ্যাক্টিভিটি লগ (এখন database.db-এর "activity_logs" সেকশন — আলাদা .json ফাইল নেই) ====================
...
def _build_services_keyboard(services):
    # আগে প্রতিটা Service আলাদা রো-তে ছিল (একটার নিচে একটা — লম্বা তালিকা হয়ে যেত)।
    # এখন দুইটা করে এক লাইনে (ডাবল লাইন) — তালিকা অর্ধেক লম্বা, স্ক্রল কম লাগবে।
    svc_buttons = []
    for i, svc in enumerate(services):
        ...
        svc_buttons.append(InlineKeyboardButton(label, callback_data=f"svc_{i}", style=color))

    buttons = [svc_buttons[j:j+2] for j in range(0, len(svc_buttons), 2)]
    # CUSTOM RANGE — আলাদা রঙে (danger = লাল/বাদামি), নিজের একক রো-তে
...
async def monitor_loop(app):
    # লুপের বাইরে একবার ডাটা লোড করে মেমোরিতে রাখা হলো (CPU বাঁচবে)
    ...
    while True:
        try:
            ...
            if "data" in res and "otps" in res["data"]:
                ...
                # নতুন ওটিপি এসেছে কিনা তা চেক করার ফ্ল্যাগ
                has_new_data = False 
                ...
                for otp in otps:
                    ...
                    if (num in active_numbers and
                        ...
                        group_buttons = InlineKeyboardMarkup([
                            [
                                InlineKeyboardButton("‼️ PANEL", url=PANEL_LINK, style="danger"),
